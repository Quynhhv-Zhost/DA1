...
=======
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h2 class="text-primary mb-0">📋 Danh sách người dùng</h2>
        <form method="GET" action="" class="d-flex align-items-center gap-2" style="min-width: 350px; max-width: 450px;">
            <input type="hidden" name="url" value="user/index">
            <input type="text" name="keyword" class="form-control" placeholder="🔍 Tìm username..." value="<?= htmlspecialchars($keyword ?? '') ?>">
                    ...
                    <?php if (!empty($admins)) : ?>
                        <?php foreach ($admins as $user) : ?>
                            <tr class="text-center">
                                ...
                                <td>
                                    <a href="?url=user/edit/<?= $user['id'] ?>" class="btn btn-sm btn-warning me-1">✏️ Sửa</a>
                                    <a href="?url=user/delete/<?= $user['id'] ?>" class="btn btn-sm btn-danger" onclick="return confirm('Bạn có chắc chắn muốn xóa người dùng này?')">🗑️ Xóa</a>
                                </td>
                            </tr>
                        <?php endforeach; ?>
                    <?php else : ?>
